	...
	describe "serviced locations" do
		...
		describe "for web" do
			...
			it "should be a relation" do
				expect(Location.serviced user, default_level).to be_an(ActiveRecord::Relation)
			end
			
			it "should list general and serviced but not without assigned services" do
				locations = Location.serviced user, default_level
		...
		describe "general, visible for all" do
			describe "asamblea" do
				...
				it "should be visible for all" do
					...
					expect(b1.map_elements(default_level)).to match_array([asamblea])
					expect(btp.map_elements(default_level)).to match_array([asamblea])
					expect(d1.map_elements(default_level)).to match_array([asamblea])
					expect(per.map_elements(default_level)).to match_array([asamblea])
					expect(soc.map_elements(default_level)).to match_array([asamblea])
				end
			end
			...
			describe "cuap" do
				...
				it "should be visible for all" do
					...
					expect(b1.map_elements(default_level)).to match_array([cuap])
					expect(btp.map_elements(default_level)).to match_array([cuap])
					expect(d1.map_elements(default_level)).to match_array([cuap])
					expect(per.map_elements(default_level)).to match_array([cuap])
					expect(soc.map_elements(default_level)).to match_array([cuap])
				end
			end
			...
			describe "hospital" do
				...
				it "should be visible for all" do
					...
					expect(b1.map_elements(default_level)).to match_array([hospital])
					expect(btp.map_elements(default_level)).to match_array([hospital])
					expect(d1.map_elements(default_level)).to match_array([hospital])
					expect(per.map_elements(default_level)).to match_array([hospital])
					expect(soc.map_elements(default_level)).to match_array([hospital])
				end
			end
			...
			describe "nostrum" do
				...
				it "should be visible for all" do
					...
					expect(b1.map_elements(default_level)).to match_array([nostrum])
					expect(btp.map_elements(default_level)).to match_array([nostrum])
					expect(d1.map_elements(default_level)).to match_array([nostrum])
					expect(per.map_elements(default_level)).to match_array([nostrum])
					expect(soc.map_elements(default_level)).to match_array([nostrum])
				end
			end
		end
		describe "don't require service" do
			...
			describe "salvamento" do
				...
				it "should be visible for acu, per" do
					...
					expect(b1.map_elements(default_level)).to match_array([])
					expect(btp.map_elements(default_level)).to match_array([])
					expect(d1.map_elements(default_level)).to match_array([])
					expect(per.map_elements(default_level)).to match_array([salvamento])
					expect(soc.map_elements(default_level)).to match_array([])
				end
			end
			...
			describe "gasolinera" do
				...
				it "should be visible for b1, d1, btp" do
					...
					expect(asi.map_elements(default_level)).to match_array([])
					expect(b1.map_elements(default_level)).to match_array([gasolinera])
					expect(btp.map_elements(default_level)).to match_array([gasolinera])
					expect(d1.map_elements(default_level)).to match_array([gasolinera])
					expect(per.map_elements(default_level)).to match_array([])
					expect(soc.map_elements(default_level)).to match_array([])
				end
			end
		end
		...
		describe "require a service" do
			...
			describe "terrestre" do
				...
				it "should be visible for asi" do
					expect(acu.map_elements(default_level)).to match_array([])
					expect(asi.map_elements(default_level)).to match_array([terrestre])
					expect(b1.map_elements(default_level)).to match_array([])
					expect(btp.map_elements(default_level)).to match_array([])
					expect(d1.map_elements(default_level)).to match_array([])
					expect(per.map_elements(default_level)).to match_array([])
			...
			describe "maritimo" do
				...
				it "should be visible only for acu, per" do
					expect(acu.map_elements(default_level)).to match_array([maritimo])
					expect(asi.map_elements(default_level)).to match_array([])
					expect(b1.map_elements(default_level)).to match_array([])
					expect(btp.map_elements(default_level)).to match_array([])
					expect(d1.map_elements(default_level)).to match_array([])
					expect(per.map_elements(default_level)).to match_array([maritimo])
					expect(soc.map_elements(default_level)).to match_array([])
				end
			end
			...
			describe "adaptadas" do
				...
				it "should be visible only for soc" do
					...
					expect(d1.map_elements(default_level)).to match_array([])
					expect(per.map_elements(default_level)).to match_array([])
					expect(soc.map_elements(default_level)).to match_array([adaptadas])
				end
			end
			...
			describe "bravo" do
				...
				it "should be visible only for asi" do
					expect(acu.map_elements(default_level)).to match_array([])
					expect(asi.map_elements(default_level)).to match_array([bravo])
					expect(b1.map_elements(default_level)).to match_array([])
					expect(btp.map_elements(default_level)).to match_array([])
					expect(d1.map_elements(default_level)).to match_array([])
					expect(per.map_elements(default_level)).to match_array([])
